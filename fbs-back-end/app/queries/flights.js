...
module.exports = Object.freeze({
    ...
    FLIGHT_ARRIVAL_SCHEDULE_LIST: 'select `t_airports`.`id`,`t_airports`.`name`,`t_airports`.`code`,`t_airports`.`country_code`, `t_airports`.`city` from t_airports where `t_airports`.`id` IN (select distinct `t_flight_schedules`.`arrival_id` from t_flight_schedules where `t_flight_schedules`.`arrival_id` '
});